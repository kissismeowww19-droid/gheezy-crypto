...
if __name__ == "__main__":
    ...
    @pytest.mark.asyncio
    async def test_get_signal_params_uses_cached_data(self, analyzer, mock_whale_tracker):
        """Test that get_signal_params uses cached data from analyze_coin to prevent second pass."""
        # Mock the necessary methods
        with patch.object(analyzer, 'get_market_data', new_callable=AsyncMock) as mock_market_data:
            ...
            # Simulate analyze_coin storing signal_data
            analyzer._last_signal_data['BTC'] = {
                'signal_data': {
                    'raw_direction': 'long',
                    'probability': 65.0,
                    'sr_levels': {
                        'resistances': [
                            {'price': 51000.0, 'strength': 5},
                            {'price': 52000.0, 'strength': 3}
                        ],
                        'supports': [
                            {'price': 49000.0, 'strength': 4}
                        ]
                    }
                },
                'market_data': {
                ...
                'timestamp': analyzer.time.time() if hasattr(analyzer, 'time') else __import__('time').time()
            }
            
            # Call get_signal_params - it should use cached data
            ...
            # Verify that calculate_signal was NOT called (no second pass)
            # The market_data should be retrieved from cache, not recalculated
            
    @pytest.mark.asyncio
    async def test_get_signal_params_falls_back_without_cache(self, analyzer, mock_whale_tracker):
        """Test that get_signal_params falls back to simplified analysis when no cache exists."""
        # Clear any cached data
        analyzer._last_signal_data.clear()
        
        # Mock necessary methods for fallback
        with patch.object(analyzer, 'get_market_data', new_callable=AsyncMock) as mock_market_data, \
             patch.object(analyzer, 'calculate_signal', new_callable=AsyncMock) as mock_calculate_signal, \
             patch.object(analyzer, 'calculate_technical_indicators', new_callable=AsyncMock) as mock_tech, \
             patch.object(analyzer.data_source_manager, 'get_ohlcv_data', new_callable=AsyncMock) as mock_ohlcv:
            ...
            mock_calculate_signal.return_value = {
                'raw_direction': 'short',
                'probability': 55.0
            }
            
            # Call get_signal_params without cache
            result = await analyzer.get_signal_params('BTC')
            
            # Verify fallback was used
            assert result is not None
            assert mock_calculate_signal.called  # Should do simplified analysis
