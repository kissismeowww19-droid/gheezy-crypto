...
class RocketHunterAnalyzer:
    ...
    async def scan_all_coins(self) -> List[Dict]:
        ...
        logger.info(
            "Rocket Hunter: scanning from 4 sources (Binance + CoinLore + CoinPaprika + CoinGecko)"
        )
        ...
        logger.info(
            f"Rocket Hunter: total {len(all_coins)} unique coins "
    ...
    async def get_top5(self) -> Tuple[List[Dict], int, int, float]:
        ...
        logger.info(
            f"Rocket Hunter found {len(scored_coins)} rockets from {filtered_count} coins"
        )
    ...
    def format_message(
        ...
        for idx, rocket in enumerate(top5, 1):
            ...
            # Цена
            price_str = (
                self._format_price(rocket["price"])
                .replace("$", "\\$")
                .replace(",", "\\,")
                .replace(".", "\\.")
            )
